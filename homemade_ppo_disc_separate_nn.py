...
from config import HYPER_PARAMS_CLASSIC as HYPER_PARAMS
    ...
    for i in range(TRAINING_ROUNDS):
        ...
        for update in range(1, num_updates + 1):
            ...
            for step in range(0, HYPER_PARAMS["ENV_TIMESTEPS"]):
                ...
                next_obs, reward, next_terminal, _, info = envs.step(
                    action.cpu().numpy()
                )
            ...
            var_y = np.var(y_true)
            explained_var = (
                np.nan if var_y == 0 else 1 - np.var(y_true - y_predict) / var_y
            )
